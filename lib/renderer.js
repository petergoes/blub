...
const renderDomFromTemplate = (module, attributes, modules) => {
	...
	if (replacedDom.length !== 1) throw new Error(`Module ${module.id} does not contain a single root node.`);
	return replacedDom;
}

const renderModule = (module, modules) => {
	const dom = renderDomFromTemplate(module, module.attributes, modules);
